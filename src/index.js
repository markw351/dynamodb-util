  ...
  if (typeof $value == 'object') {
    ...
    for (var i in $value) {
      if (hasOwnProperty.call($value, i)) {
        ...
        if (val !== undefined)
          // when empty string is replaced with undefined
          to_ret.M[i] = val
      }
    }
    return to_ret
...
DynamoUtil.parse = function (v) {
  ...
  if (hasOwnProperty.call(v, 'N')) return parseFloat(v.N)

  if (hasOwnProperty.call(v, 'BOOL')) return v.BOOL

  if (hasOwnProperty.call(v, 'NULL')) return null
  ...
  if (hasOwnProperty.call(v, 'BS')) {
    if (DynamoUtil.config.binaryset_parse_as_set)
      return new Set(
        v.BS.map(function (el) {
          return el
        })
      )
  ...
  if (hasOwnProperty.call(v, 'M')) {
    var normal = {}
    for (var i in v.M) {
      if (hasOwnProperty.call(v.M, i)) {
...
DynamoUtil.normalizeItem = function ($item) {
  ...
  for (var key in $item) {
    if (hasOwnProperty.call($item, key)) {
      if (hasOwnProperty.call($item[key], 'S')) normal[key] = $item[key]['S']

      if (hasOwnProperty.call($item[key], 'N')) normal[key] = +$item[key]['N']

      if (hasOwnProperty.call($item[key], 'BOOL'))
        normal[key] = $item[key]['BOOL']

      if (hasOwnProperty.call($item[key], 'NULL')) normal[key] = null

      if (hasOwnProperty.call($item[key], 'B')) normal[key] = $item[key]['B']

      if (hasOwnProperty.call($item[key], 'SS')) normal[key] = $item[key]['SS']

      if (hasOwnProperty.call($item[key], 'NS')) {
        normal[key] = []
      ...
      if (hasOwnProperty.call($item[key], 'L')) {
        normal[key] = []
        for (var i in $item[key]['L']) {
          if (hasOwnProperty.call($item[key]['L'], i)) {
      ...
      if (hasOwnProperty.call($item[key], 'M')) {
        normal[key] = {}
        for (var i in $item[key]['M']) {
          if (hasOwnProperty.call($item[key]['M'], i)) {
            normal[key][i] = DynamoUtil.normalizeItem({
              key: $item[key]['M'][i],
            }).key
          }
  ...
  for (var key in $expected) {
    if (hasOwnProperty.call($expected, key)) {
      ...
      if (
        ...
        $expected[key] instanceof DynamoUtil.Raw
      ) {
        anormal[key] = $expected[key].data
      } else if (
        hasOwnProperty.call($expected[key], 'value2') &&
        $expected[key].value2 !== undefined
      ) {
        anormal[key] = {
          ComparisonOperator: $expected[key].operator,
          AttributeValueList: [
            DynamoUtil.stringify($expected[key].value),
            DynamoUtil.stringify($expected[key].value2),
          ],
        }
      } else {
        anormal[key] = {
...
DynamoUtil.toSQLJSON = function (o, is_list) {
  ...
  Object.keys(o).map(function (k) {
    ...
    if (o[k].hasOwnProperty('SS'))
      oeach.push(
        "'" + k + "':" + 'new StringSet(' + JSON.stringify(o[k].SS) + ')'
      )
